...
import { Message } from "@/app/backend/models";
import MessageCard from "@/app/components/message_card";
import { AUTO_REPLIES, currentUser, mockMessages } from "@/app/test_items/test_data";
...
export default function ChatScreen() {
  ...
  const send = useCallback(() => {
    ...
    if (!text) return;
    const outgoing: Message = new Message(Date.now(), user.profile_id, text, dateToCustomDate(new Date(Date.now())));
    ...
    const replyTimer = setTimeout(() => {
      const reply: Message = new Message(
        ...
        dateToCustomDate(new Date(Date.now()))
      );
  ...
  const handleJoin = useCallback(() => {
    ...
    const confirmMsg: Message = new Message(
      Date.now(),
      user.profile_id,
      `Just RSVP'd to ${park} — see you there! 🐾`,
      dateToCustomDate(new Date(Date.now()))
    );
    ...
    setTimeout(() => {
      const replyMsg: Message = new Message(
        Date.now() + 1,
        friendId,
        `Yay!! Can't wait to see you and Buddy 🐶🎉`,
        dateToCustomDate(new Date(Date.now()))
      );
